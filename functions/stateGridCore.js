...
const request$1 = async (request = {} || '', option = {}) => {
  ...
  const { url: url, ...rest } = request;
  return await got[method](url, rest).then(
    e => (
...
class Store {
  constructor(NAMESPACE) {
    ...
    this.env = getEnv();
    const dir = path.join(__dirname, '../db/state-grid');
    try {
      fs.mkdirSync(dir, { recursive: true });
    } catch (_) {}
    const safe = String(NAMESPACE || 'default').replace(/[^a-zA-Z0-9_-]/g, '_');
  ...
  _save() {
    require('fs').writeFileSync(this.file, JSON.stringify(this._cache));
  }
  get(e) {
    if ('Node.js' !== this.env) return null;
    const v = this._cache[e];
...
const notify = (e = '', o = '', r = '', s = {}) => {
  ...
  NAMESPACE =
    ...
    require('crypto')
      .createHash('sha256')
      .update(String(process.env.WSGW_USERNAME || 'default'))
      .digest('hex')
      .slice(0, 24),
...
async function sendMsg(e, eleBill, dayList, monthElecQuantity) {
  const host =
    (isNode() ? process.env.mqtt_host : store.get('95598_mqtt_host')) || '',
    port =
      (isNode() ? process.env.mqtt_port : store.get('95598_mqtt_port')) || '',
    mqtt_username = (isNode() ? process.env.mqtt_username : store.get('95598_mqtt_username')) || '',
    mqtt_password = (isNode() ? process.env.mqtt_password : store.get('95598_mqtt_password')) || '';

  if (!host || !port) {
    console.log('[state-grid] 未配置 mqtt_host / mqtt_port，跳过 MQTT');
    return;
  }

  const mqtt = require('mqtt')
  const clientId = 'mqtt_sg_' + Date.now() + '_' + Math.random().toString(36).slice(2, 8)
  ...
  const topic =
    'nodejs/state-grid/' + (eleBill.consNo || eleBill.consNo_dst || 'unknown')
  let data = eleBill;
